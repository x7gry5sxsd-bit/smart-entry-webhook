...
Версия 2.2: Расширенный regex (все Solana CA) + умный CA detection.
“””
...
# Все Solana CA: 32-44 base58 символа

# Убрали ограничение на “pump” — теперь ловим все токены

CA_REGEX = re.compile(r’\b([1-9A-HJ-NP-Za-km-z]{32,44})\b’)

# Слова которые НЕ являются CA (но могут попасть под base58 regex)
...
def extract_ca(text):
“”“Извлечь Solana CA из текста.

```
Приоритеты:
1. Самое длинное совпадение (44 символа = pump или стандарт)
2. Заканчивающийся на "pump" предпочтительнее (более вероятный pump.fun токен)
"""
if not text:
    return None
...
# Фильтруем: длина ≥ 32, не входит в exclude
candidates = [
    m for m in matches 
    if 32 <= len(m) <= 44 
    and m.upper() not in EXCLUDE_WORDS
    and not m.isdigit()  # не чистые числа
]

if not candidates:
    return None

# Приоритет: pump-токены первыми (если есть)
...
rug_score = 0
rug_flags = “N/A”
lp_locked = 100  # Нейтрально: если неизвестно - не блокируем
...
if not ca:
    # Не нашли CA - возвращаем нейтральные данные (не блокирующие)
    return {
        "enrichment": "\n=== ON-CHAIN ===\nRUG_LEVEL: UNKNOWN\nRUG_SCORE: 0\nRUG_FLAGS: N/A\nLP_LOCKED: 100\nDEX_STATUS: UNAVAILABLE\nVOL_5M: 0\nVOL_1H: 0\nVOL_ACCEL: 0\nBS_ONCHAIN: 0\nBUYS_5M: 0\nSELLS_5M: 0\nPRICE_5M: 0\n===============",
    }
